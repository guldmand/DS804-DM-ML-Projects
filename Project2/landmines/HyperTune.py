...
# Læs data og forbered
data_path = "./data/land_mines.csv"  # tilpas evt. sti
...
def hypertune_on_train(X_train, y_train, cv):
    ...
    gs_knn = GridSearchCV(knn, knn_params, cv=cv)
    gs_knn.fit(X_train, y_train)
    print("  KNN:", gs_knn.best_params_, "| Score:", round(gs_knn.best_score_, 3))
    # 2. DecisionTree
    dt_params = {"max_depth": [2, 4, 6, 8, 10, 12, 15, 20]}
    dt = DecisionTreeClassifier(random_state=1)
    gs_dt = GridSearchCV(dt, dt_params, cv=cv)
    ...
    gs_rf = GridSearchCV(rf, rf_params, cv=cv)
    ...
    gs_mlp = GridSearchCV(mlp, mlp_params, cv=cv)
    gs_mlp.fit(X_train, y_train)
    print("  MLP:", gs_mlp.best_params_, "| Score:", round(gs_mlp.best_score_, 3))
    # 5. SVM
    svm_params = {"C": [0.1, 1, 10], "kernel": ["linear", "rbf"]}
    svm = SVC(random_state=1)
    gs_svm = GridSearchCV(svm, svm_params, cv=cv)
...
for split in split_percents:
    ...
    # her bruger vi CV=5 for grid search som default for ALLE algoritmer, inkl. MLP etc.
    hypertune_on_train(X_train, y_train, cv=5)

# --------- K-Fold tuning (hypertune på hele datasættet for hvert k) ----------
print("\\n------ K-Fold (kun hypertuning, ikke test) ------")
for k in range(2, 11):
    print(f"=== K-Fold Cross-Validation, K={k} ===")
    # Her bruger vi cv=k, så GridSearchCV bruger K-fold som ønsket
